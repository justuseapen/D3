class TracsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @trac.save
        format.html { redirect_to sesh_path(params[:id]), notice: 'Trac was successfully created.' }
        format.json { render action: 'show', status: :created, location: @trac }
      else
  ...
  def destroy
    @trac.destroy
    respond_to do |format|
      format.html { redirect_to tracs_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def trac_params
      params.require(:trac).permit(:sesh_id)
    end
end
